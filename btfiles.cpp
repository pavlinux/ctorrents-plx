...
int btFiles::_btf_close_oldest() {
    ...
    for (pbf_n = m_btfhead; pbf_n; pbf_n = pbf_n->bf_next) {
        if (!pbf_n->bf_flag_opened)
            continue; // file not been opened.
        if (!pbf_close
                || pbf_n->bf_last_timestamp < pbf_close->bf_last_timestamp)
            pbf_close = pbf_n;
    }
    if (!pbf_close)
        return -1;
...
int btFiles::_btf_open(BTFILE * pbf, const int iotype) {
    ...
    if (pbf->bf_flag_opened) {
        if (pbf->bf_flag_readonly && iotype)
            ...
            return 0; // already open in a usable mode
    }
    ...
    if (m_directory) {
        if (MAXPATHLEN <=
                snprintf(fn, MAXPATHLEN, "%s%c%s", m_directory, PATH_SP,
                pbf->bf_filename)) {
            errno = ENAMETOOLONG;
            return -1;
        }
...
ssize_t btFiles::IO(char *buf, uint64_t off, size_t len, const int iotype) {
    ...
    if ((off + (uint64_t) len) > m_total_files_length) {
        CONSOLE.Warning(1,
                "error, data offset %llu length %lu out of range",
    ...
    for (; len;) {
...
#endif
            CONSOLE.Warning(1,
                    "error, failed to seek to %llu on file \"%s\":  %s",
        ...
        nio =
                (len < pbf->bf_length - pos) ? len : (pbf->bf_length - pos);
        ...
        if (0 == iotype) {
            ...
            if (1 != fread(buf, nio, 1, pbf->bf_fp)
                    && ferror(pbf->bf_fp)) {
                CONSOLE.Warning(1,
                        "error, read failed at %llu on file \"%s\":  %s",
                        (unsigned long long) pos,
                        pbf->bf_filename,
                        strerror(errno));
                return -1;
            ...
            if (1 != fwrite(buf, nio, 1, pbf->bf_fp)) {
                CONSOLE.Warning(1,
                        "error, write failed at %llu on file \"%s\":  %s",
                        (unsigned long long) pos,
                        pbf->bf_filename,
                        strerror(errno));
                return -1;
            ...
            if (fflush(pbf->bf_fp) == EOF) {
                CONSOLE.Warning(1,
                        "error, flush failed at %llu on file \"%s\":  %s",
                        (unsigned long long) pos,
                        pbf->bf_filename,
                        strerror(errno));
                return -1;
...
int btFiles::_btf_destroy() {
    ...
    for (pbf = m_btfhead; pbf;) {
        ...
        if (pbf->bf_fp && pbf->bf_flag_opened)
            fclose(pbf->bf_fp);
        if (pbf->bf_filename)
            delete[]pbf->bf_filename;
        if (pbf->bf_buffer)
            delete[]pbf->bf_buffer;
        delete pbf;
        pbf = pbf_next;
    ...
    while ((struct dirent *) 0 != (dirp = readdir(dp))) {
        ...
        if (stat(fn, &sb) < 0) {
            CONSOLE.Warning(1, "error, stat \"%s\" failed:  %s", fn,
                    strerror(errno));
            closedir(dp); // CID: 28181  Resource leak (RESOURCE_LEAK)
            return -1;
        }
...
int btFiles::BuildFromMI(const char *metabuf, const size_t metabuf_len,
    ...
    if (r) {
        ...
        if (decode_query(metabuf, metabuf_len, "info|length",
                (const char **) 0, (size_t *) 0, (int64_t *) 0,
                QUERY_LONG))
            return -1;
...
void btFiles::SetFilter(int nfile, BitField * pFilter, size_t pieceLength) {
    ...
    for (; p; p = p->bf_next) {
        if ((ssize_t) ++id == nfile) {
            ...
            if (stop > start
                    && 0 == (sizeBuffer + p->bf_length) % pieceLength)
                --stop;
