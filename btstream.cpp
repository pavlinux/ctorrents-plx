...
ssize_t btStream::Send_Piece(size_t idx, size_t off, char *piece_buf,
        size_t len) {
    ...
    idx = htonl(idx);
    off = htonl(off);
    if ((r = out_buffer.Put(sock, (char *) &q, H_LEN)) < 0)
        return r;
    if ((r = out_buffer.Put(sock, (char *) &t, H_BASE_LEN)) < 0)
        return r;
    if ((r = out_buffer.Put(sock, (char *) &idx, H_INT_LEN)) < 0)
        return r;
    if ((r = out_buffer.Put(sock, (char *) &off, H_INT_LEN)) < 0)
        return r;
    ...
    if (H_LEN + H_PIECE_LEN < in_buffer.Count() &&
        ...
        if (r > H_PIECE_LEN) {
            ...
            if (in_buffer.Count() >= r + H_LEN) { // have the whole message
                ...
                size_t nbytes =
                        in_buffer.Count() - H_LEN - H_PIECE_LEN;
...
int btStream::PeekNextMessage(char m) {
    ...
    base =
            in_buffer.BasePointer() + H_LEN + get_nl(in_buffer.BasePointer());
