...
class RequestQueue {
 ...
 public:
	...
	void SetNextSend(PSLICE ps) {
		rq_send = ps;
	} PSLICE GetHead() const {
		return rq_head;
	} PSLICE NextSend() const {
		return rq_send;
	} size_t GetRequestIdx() const {
		return rq_head ? rq_head->index : BTCONTENT.GetNPieces();
	} size_t GetRequestLen() const {
		return rq_head ? rq_head->length : 0;
	} void Release() {
		rq_head = rq_send = (PSLICE) 0;
	}
