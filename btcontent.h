...
public:
    ...
    int FlushFailed() const {
        return m_flush_failed ? 1 : 0;
    }
    ...
    size_t CheckedPieces() const {
        return m_check_piece;
    }
    ...
    unsigned char* GetShakeBuffer() {
        return m_shake_buffer;
    }

    unsigned char* GetInfoHash() {
        return (m_shake_buffer + 28);
    }

    unsigned char* GetPeerId() {
        return (m_shake_buffer + 48);
    }
    ...
    uint64_t GetTotalFilesLength() const {
        return m_btfiles.GetTotalLength();
    }

    uint64_t GetLeftBytes() const {
        return m_left_bytes;
    }
    ...
    BitField *GetNextFilter() const {
        return GetNextFilter((BitField *) 0);
    }
    BitField *GetNextFilter(BitField *pfilter) const;

    char *GetFilterName() const {
        return m_current_filter->name;
    }
    ...
    time_t GetSeedTime() const {
        return m_seed_timestamp;
    }

    size_t GetHashFailures() const {
        return m_hash_failures;
    }

    size_t GetDupBlocks() const {
        return m_dup_blocks;
    }

    size_t GetUnwantedBlocks() const {
        return m_unwanted_blocks;
    }

    void CountHashFailure() {
        m_hash_failures++;
    }
    void CountDupBlock(size_t len);

    void CountUnwantedBlock() {
        m_unwanted_blocks++;
    }
    ...
    size_t CacheMiss() const {
        return m_cache_miss;
    }
    // instead, "pre" does

    size_t CachePre() const {
        return m_cache_pre;
    }
