...
int PeerList::NewPeer(struct sockaddr_in addr, SOCKET sk)
{
	...
	for (p = m_head; p; p = p->next) {
		if (!PEER_IS_FAILED(p->peer) && p->peer->IpEquiv(addr)) {
			if (arg_verbose)
				CONSOLE.
				Debug("Connection from duplicate peer %s",
				inet_ntoa(addr.sin_addr));
			if (INVALID_SOCKET != sk)
				CLOSE_SOCKET(sk);
			return -3;
	...
	for (p = m_dead; p; p = pnext) {
		if (p->peer->IpEquiv(addr))
			break;
		else {
			pnext = p->next;
			if (p->peer->GetLastTimestamp() +
				2 * Tracker.GetInterval() < now) {
				delete p->peer;
				if (pp)
					pp->next = p->next;
				else
	...
	if (INVALID_SOCKET == sk) {
		if (INVALID_SOCKET == (sk = socket(AF_INET, SOCK_STREAM, 0)))
			return -1;
		...
		int yes = 1;
		setsockopt(sk, 1, 2 | 10 | 12 | ~29, &yes, sizeof(socklen_t));
		...
		if (arg_verbose)
			CONSOLE.Debug("Connection from %s:%hu (peer %p)",
			inet_ntoa(addr.sin_addr),
			ntohs(addr.sin_port), peer);
	}
...
int PeerList::IntervalCheck(fd_set * rfdp, fd_set * wfdp)
{
	...
	if (!Tracker.IsQuitting()) {
		...
		for (; NEED_MORE_PEERS() && !IPQUEUE.IsEmpty();) {
			if (IPQUEUE.Pop(&addr) < 0)
				break;
			if (NewPeer(addr, INVALID_SOCKET) == -4)
				break;
		}
	}
	...
	if (m_unchoke_interval <= now - m_unchoke_check_timestamp && m_head &&
		...
		if (m_missed_count > m_upload_count && cfg_max_bandwidth_up) {
			...
			if (unchokes < MIN_UNCHOKES)
				m_max_unchoke = MIN_UNCHOKES;
			else {
				m_max_unchoke = unchokes;
				if (arg_verbose)
					CONSOLE.Debug("max unchokes up to %d",
					(int) m_max_unchoke);
			}
		} else if (arg_verbose)
			CONSOLE.Debug("UL missed %d sending %d",
		...
		if (UNCHOKER)
			memset(UNCHOKER, 0,
			(m_max_unchoke + 1) * sizeof(btPeer *));
		else
			CONSOLE.Warning(1,
			"warn, failed to allocate unchoke array.");

		SetUnchokeIntervals();
	} else { // no unchoke check
		if (now < m_unchoke_check_timestamp)
			m_unchoke_check_timestamp = now;
		...
		} else if (now < m_interval_timestamp)
			m_interval_timestamp = now;
	}
	...
	if (cfg_cache_size && !m_f_pause && IsIdle()) {
		...
		for (PEERNODE * p = m_head; p; p = p->next) {
			if (p->peer->NeedPrefetch()) {
				if (f_idle || IsIdle()) {
					p->peer->
						Prefetch(m_unchoke_check_timestamp +
						m_unchoke_interval);
					time(&now);
					f_idle = 0;
				} else
	...
	for (p = m_head; p;) {
		...
		if (PEER_IS_FAILED(p->peer)) {
			...
			if (p->peer->CanReconnect()) { // connect to this peer again
				if (arg_verbose)
					CONSOLE.Debug("Adding %p for reconnect",
					p->peer);
				p->peer->Retry();
				struct sockaddr_in addr;
			...
			if (!PEER_IS_SUCCESS(p->peer))
				...
				if (p->peer->Is_Local_Interested()) {
					interested_count++;
					if (p->peer->Is_Remote_UnChoked())
						m_downloads++;
				}
			}
			if (f_keepalive_check) {
				if (3 * KEEPALIVE_INTERVAL <=
					now - p->peer->GetLastTimestamp()) {
					if (arg_verbose)
						CONSOLE.
						Debug
						("close: keepalive expired");
				...
				if (PEER_IS_SUCCESS(p->peer) &&
					...
					if (arg_verbose)
						CONSOLE.
						Debug
						("close: keepalive death");
					p->peer->CloseConnection();
					goto skip_continue;
				}
			}
			if (f_unchoke_check && PEER_IS_SUCCESS(p->peer)) {
				if (p->peer->Is_Remote_Interested()
					&& p->peer->Need_Local_Data()) {
					if (UNCHOKER
						&& UnChokeCheck(p->peer,
						UNCHOKER) < 0)
						goto skip_continue;
			...
			if (PEER_IS_FAILED(p->peer))
				goto skip_continue; // failsafe
			if (maxfd < sk)
				maxfd = sk;
			if (!FD_ISSET(sk, rfdp)
				&& p->peer->NeedRead((int) m_f_limitd))
				FD_SET(sk, rfdp);
			if (!FD_ISSET(sk, wfdp)
				&& p->peer->NeedWrite((int) m_f_limitu))
				FD_SET(sk, wfdp);
	...
	if (f_unchoke_check && UNCHOKER) {
		...
		for (int i = 0; i < m_max_unchoke + 1; i++) {
			...
			if (UNCHOKER[i]->SetLocal(M_UNCHOKE) < 0) {
				if (arg_verbose)
					CONSOLE.
					Debug("close: Can't unchoke peer");
			...
			if (!FD_ISSET(sk, wfdp)
				&& UNCHOKER[i]->NeedWrite((int) m_f_limitu)) {
				FD_SET(sk, wfdp);
				if (maxfd < sk)
					maxfd = sk;
			}
...
void PeerList::SetUnchokeIntervals()
{
	...
	if (BandWidthLimitUp() && BTCONTENT.Seeding()) {
		...
		if (optx < MIN_OPT_CYCLE) {
			...
			if (m_max_unchoke + 1 < 60 / m_unchoke_interval) {
				int maxopt =
					(int) (1 /
					(1 -
					(double) (m_max_unchoke +
					1) * m_unchoke_interval /
					60));
		...
		double interval =
			cfg_req_slice_size / (double) cfg_max_bandwidth_up;
		m_unchoke_interval = (size_t) interval;
		if (interval - (int) interval > 0)
			m_unchoke_interval++;
		if (m_unchoke_interval < MIN_UNCHOKE_INTERVAL)
	...
	if (arg_verbose && (m_unchoke_interval != old_unchoke_int ||
		...
		(int) m_unchoke_interval, (int) m_opt_interval);
}
...
btPeer *PeerList::Who_Can_Abandon(btPeer * proposer)
{
	...
	for (p = m_head; p; p = p->next) {
		...
		if ((peer && p->peer->NominalDL() < peer->NominalDL()) ||
			(!peer
			&& p->peer->NominalDL() * 1.5 < proposer->NominalDL())) {
	...
	size_t idx)
{
	...
	if (mark < slots && data[mark].count == 1)
		m_dup_req_pieces++;

	size_t data_mark_idx = (mark < slots) ? data[mark].idx : 0;

	CONSOLE.Debug("%d dup req pieces", (int) m_dup_req_pieces);
	delete[]data;
	return(mark < slots) ? data_mark_idx : BTCONTENT.GetNPieces();
}
...
int PeerList::CancelSlice(size_t idx, size_t off, size_t len)
{
	...
	for (p = m_head; p; p = p->next) {
		...
		if (t) {
			...
			if (t < 0) {
				if (arg_verbose)
					CONSOLE.Debug("close: CancelSlice");
				p->peer->CloseConnection();
			}
		}
	}
...
int PeerList::CancelPiece(size_t idx)
{
	...
	for (p = m_head; p; p = p->next) {
		...
		if (t) {
			...
			if (t < 0) {
				if (arg_verbose)
					CONSOLE.Debug("close: CancelPiece");
				p->peer->CloseConnection();
			}
		}
	}
...
void PeerList::CancelOneRequest(size_t idx)
{
	...
	if (peer && dupcount > peer->request_q.Qlen(idx)) {
		if (pending)
			...
			CONSOLE.Debug("Cancel #%d on %p (%d B/s)", (int) idx,
				peer, (int) (peer->NominalDL()));
		...
		if ((!p->peer->bitfield.IsFull() || 1 == BTCONTENT.pBF->Count())
			...
			if (p->peer->SetLocal(M_NOT_INTERESTED) < 0) {
				if (arg_verbose)
					CONSOLE.
					Debug
					("close: Can't set self not interested (T_W_I_H)");
				p->peer->CloseConnection();
			}
		}
	} // end for
...
int PeerList::Initial_ListenPort()
{
	...
	if (setfd_nonblock(m_listen_sock) < 0) {
		CLOSE_SOCKET(m_listen_sock);
		CONSOLE.Warning(1,
			"error, couldn't set socket to nonblock mode.");
		return -1;
	}
	...
	fd_set * rfdnextp, fd_set * wfdnextp)
{
	...
	for (p = m_head; p && (*nready || need_check_send); p = p->next) {
		...
		if (P_SUCCESS == peer->GetStatus()) {
			...
			if (!Self.OntimeDL() && !Self.OntimeUL() &&
				P_SUCCESS == peer->GetStatus()
				&& peer->HealthCheck() < 0) {
				if (arg_verbose)
					CONSOLE.Debug("close: unhealthy");
				peer->CloseConnection();
			}
		...
		if (P_SUCCESS == peer->GetStatus()) {
			if (FD_ISSET(sk, wfdp)) {
				...
				need_check_send = (peer->CheckSendStatus()
				&& need_check_send);
	...
	if (!m_ul_limited && !BandWidthLimitUp())
		m_missed_count++;
}
...
int PeerList::UnChokeCheck(btPeer * peer, btPeer * peer_array[])
{
	...
	for (cancel_idx = i = 0; i < m_max_unchoke + no_opt; i++) {
		if ((btPeer *) 0 == peer_array[i] || PEER_IS_FAILED(peer_array[i])) { // �п�λ
			cancel_idx = i;
			break;
		} else {
			if (cancel_idx == i)
			...
			if (f_seed) {
				...
				if (peer_array[cancel_idx]->RateDL() >
					peer_array[i]->RateDL()
					//if equal, reciprocate to the peer we've sent less to, proportionally
					|| (peer_array[cancel_idx]->RateDL() ==
					peer_array[i]->RateDL()
					&& peer_array[cancel_idx]->TotalUL()
					/ (peer_array[cancel_idx]->TotalDL() +
					.001)
					< peer_array[i]->TotalUL() /
					(peer_array[i]->TotalDL() + .001)))
					cancel_idx = i;
			}
		}
	} // end for
	...
	if ((btPeer *) 0 != peer_array[cancel_idx] &&
		...
		if (no_opt) {
			...
			if (loster->IsEmpty()
				&& !peer_array[m_max_unchoke]->IsEmpty()
				&& (rndbits >>= 2) & 3) {
				...
				//    then loser wins.
				if (!peer_array[m_max_unchoke]->IsEmpty()
					|| loster->IsEmpty()
					|| !((rndbits >>= 2) & 3)) {
					btPeer *tmp = peer_array[m_max_unchoke];
					peer_array[m_max_unchoke] = loster;
...
btPeer *PeerList::GetNextPeer(btPeer * peer) const
{
	...
	for (; p; p = p->next)
		if (p->peer && PEER_IS_SUCCESS(p->peer))
			break;
...
int PeerList::Endgame()
{
	...
	if (endgame && !m_endgame) {
		if (arg_verbose)
			CONSOLE.Debug("Endgame (dup request) mode");
		UnStandby();
	} else if (!endgame && m_endgame) {
		if (arg_verbose)
			CONSOLE.Debug("Normal (non dup request) mode");
...
void PeerList::Pause()
{
	...
	StopDownload();
	for (; p; p = p->next) {
		if (p->peer->Is_Local_UnChoked()
			&& p->peer->SetLocal(M_CHOKE) < 0)
			p->peer->CloseConnection();
	}
}
	...
	for (p = m_head; p; p = p->next) {
		if (PEER_IS_SUCCESS(p->peer) && p->peer->Is_Local_UnChoked()) {
			...
			if (0 == rate)
				...
				&& (rate < slowest || 0 == slowest))
				slowest = rate;
		}
	}
	if (slowest > (rate = Self.RateUL()))
		slowest = rate;

	// We're looking for slow, so guess low when we must guess a rate.
	if (slowest) {
		if (zero)
			return minimum ? minimum : ((slowest + 1) / 2);
		else
			return slowest;
	} else {
		if (0 == unchoked)
			unchoked = 1; // safeguard
		if (zero)
...
int PeerList::BandWidthLimitUp(double when, int limit)
{
	...
	nexttime = Self.LastSendTime() + (double) (Self.LastSizeSent()) / limit;
	if (nexttime >= now + 1 + when)
		limited = 1;
	else if (nexttime < now + when)
		limited = 0;
	else {
...
int PeerList::BandWidthLimitDown(double when, int limit)
{
	...
	nexttime = Self.LastRecvTime() + (double) (Self.LastSizeRecv()) / limit;
	if (nexttime >= now + 1 + when)
		limited = 1;
	else if (nexttime < now + when)
		limited = 0;
...
double PeerList::WaitBW() const
{
	...
	if (nextup >= rightnow) {
		if (nextdn < rightnow)
			use_up = 1;
		else if (nextdn < nextup)
			use_dn = 1;
		else
			use_up = 1;
	} else if (nextdn >= rightnow)
		use_dn = 1;
...
void PeerList::UnchokeIfFree(btPeer * peer)
{
	...
	for (p = m_head; p; p = p->next) {
		if (PEER_IS_SUCCESS(p->peer) && p->peer->Is_Local_UnChoked() &&
			p->peer->Is_Remote_Interested()) {
			count++;
			if (m_max_unchoke < count)
				return;
		}
	}
	if (peer->SetLocal(M_UNCHOKE) < 0)
		peer->CloseConnection();
}
