...
int ConStream::SameDev(ConStream * master) const {
    ...
    if (master == this || Fileno() == master->Fileno())
        return 1;
    else if (Fileno() < 0 || master->Fileno() < 0)
        return 0;

    if (!fstat(Fileno(), &sbone) && !fstat(master->Fileno(), &sbtwo))
        return (sbone.st_dev == sbtwo.st_dev
            && sbone.st_ino == sbtwo.st_ino) ? 1 : 0;
    else
        return 0;
}
...
#endif
    if (r < 0) {
        Error(1, "Error preserving terminal mode on fd %d:  %s",
...
#endif
    if (r < 0) {
        Error(1, "Error restoring terminal mode on fd %d:  %s",
                Fileno(), strerror(errno));
    }
}
...
Console::Console() {
    ...
    if (STATUSLINES > i) {
        fprintf(stderr,
                "Unassigned status line in Console() constructor!\n");
        exit(1);
...
void Console::User(fd_set * rfdp, fd_set * wfdp __attribute__((unused)),
        int *nready, fd_set * rfdnextp, fd_set * wfdnextp __attribute__((unused))) {
        ...
        if (K_LINES == m_streams[O_INPUT]->GetInputMode()) { // command parameter
            ...
            switch (c) {
                ...
                case '?': // help
                    ...
                    Interact(" %-9s%-30s %-9s%s", "[Esc/0]",
                            "Operator menu", "m[+/-]",
                            "Adjust min peers count");
                    Interact(" %-9s%-30s %-9s%s", "d[+/-]",
                            "Adjust download limit", "M[+/-]",
                            "Adjust max peers count");
                    Interact(" %-9s%-30s %-9s%s", "u[+/-]",
                            "Adjust upload limit", "C[+/-]",
                            "Adjust max cache size");
                    Interact(" %-9s%-30s %-9s%s", "n",
                            "Download specific files", "S",
                            "Set/change CTCS server");
                    Interact(" %-9s%-30s %-9s%s", "e[+/-]",
                            "Adjust seed exit time", "v",
                            "Toggle verbose mode");
                    Interact(" %-9s%-30s %-9s%s", "E[+/-]",
                            "Adjust seed exit ratio", "Q", "Quit");
                    Interact(" %-9s%-30s %-9s%s", "X",
                            "Completion command", "", "");
                    break;
                ...
                case 'v': // verbose
                    if (arg_verbose
                            && !m_streams[O_INPUT]->
                            SameDev(m_streams[O_DEBUG]))
                        Debug("Verbose output off");
                    arg_verbose = !arg_verbose;
                    Interact("Verbose output %s",
                            arg_verbose ? "on" : "off");
                    break;
                case 'Q': // quit
                ...
                case '-': // decrease value
                    ...
                    switch (pending) {
                        ...
                        case 'E':
                            ...
                            break;
                        case 'm':
                            value = (int) cfg_min_peers;
                            value += inc;
                            cfg_min_peers =
                                    (value < 1) ? 1 : (size_t) value;
                            if (cfg_min_peers > cfg_max_peers)
                                cfg_min_peers = cfg_max_peers;
                            break;
                        case 'M':
                            value = (int) cfg_max_peers;
                            value += inc;
                            cfg_max_peers =
                                    (value <
                                    (int) cfg_min_peers) ? cfg_min_peers
                                    : (size_t) value;
                            if (cfg_max_peers > 1000)
                                cfg_max_peers = 1000;
                            break;
                        case 'C':
                            value = (int) cfg_cache_size;
                            value += inc;
                            cfg_cache_size =
                                    (value < 0) ? 0 : (size_t) value;
            ...
            switch (pending) {
                case 'd':
                    InteractU("DL Limit: %d B/s ",
                            (int) cfg_max_bandwidth_down);
                    break;
                case 'u':
                    InteractU("UL Limit: %d B/s ",
                            (int) cfg_max_bandwidth_up);
                    break;
                case 'e':
...
int Console::OperatorMenu(const char *param) {
    ...
    if (0 == oper_mode) {
        ...
        for (int i = 0; i < STATUSLINES; i++) {
            ...
            Interact(" %c%d) %s",
                    (i == m_status_format) ? '*' : ' ', ++n_opt,
                    buffer);
        }
        Interact(" Other options:");
        Interact(" %2d) View detailed status", ++n_opt);
        if (WORLD.IsPaused())
            Interact(" %2d) Resume (continue upload/download)",
                ++n_opt);
        else
            Interact(" %2d) Pause (suspend upload/download)",
                ++n_opt);
        if (!arg_daemon)
            Interact(" %2d) Become daemon (fork to background)",
                ++n_opt);
        Interact(" %2d) Update tracker stats & get peers", ++n_opt);
        Interact(" %2d) Restart (recover) the tracker session",
                ++n_opt);
        ...
        if (sel <= O_NCHANNELS + 1) { // change i/o channel
            ...
            Interact
                    ("Failed hashes: %d    Dup blocks: %d    Unwanted blocks: %d",
                    (int) (BTCONTENT.GetHashFailures()),
                    (int) (BTCONTENT.GetDupBlocks()),
                    (int) (BTCONTENT.GetUnwantedBlocks()));
            Interact("");
            Interact("Peers: %d   Min: %d   Max: %d",
                    (int) (WORLD.GetPeersCount()),
                    (int) cfg_min_peers, (int) cfg_max_peers);
            Interact("Listening on: %s", WORLD.GetListen());
            Interact("Announce URL: %s", BTCONTENT.GetAnnounce());
            Interact("");
            Interact
                    ("Ratio: %.2f   Seed time: %luh   Seed ratio: %.2f",
                    (double) (Self.TotalUL()) /
            ...
            Interact("Cache in use: %dKB  Wants: %dKB  Max: %dMB",
                    ...
                    (int) (BTCONTENT.CacheSize() / 1024),
                    (int) cfg_cache_size);
            if (arg_ctcs)
                Interact("CTCS Server: %s", arg_ctcs);
            if (arg_verbose)
                cpu();
            oper_mode = 0;
            return 1;
            ...
            if (Tracker.GetStatus() == T_FREE)
                Tracker.Reset(15);
            else
                Interact
                    ("Already connecting, please be patient...");
            oper_mode = 0;
            return 1;
...
int Console::ChangeChannel(int channel, const char *param, int notify) {
    ...
    if (0 == strcasecmp(param, m_stdout.GetName()))
        dest = &m_stdout;
    else if (0 == strcasecmp(param, m_stderr.GetName()))
        dest = &m_stderr;
    else if (0 == strcasecmp(param, m_stdin.GetName()))
        dest = &m_stdin;
    else if (0 == strcasecmp(param, m_off.GetName()))
        dest = &m_off;
    else {
        for (int i = 0; i <= O_NCHANNELS; i++) {
            if (channel != i
                    && 0 == strcmp(param, m_streams[i]->GetName())
                    && m_streams[i]->GetMode() ==
                    ((channel == O_INPUT) ? 0 : 1)) {
                dest = m_streams[i];
                break;
        ...
        if (!dest) {
            ...
            if (dest) {
                ...
                Interact("Failed to allocate memory.");
        }
    }
    if (dest) {
        if (O_INPUT == channel)
            m_oldfd = m_streams[channel]->Fileno();
        if (m_streams[channel] != &m_stdout
                && m_streams[channel] != &m_stderr
                && m_streams[channel] != &m_stdin
                && m_streams[channel] != &m_off) {
            int in_use = 0;
            for (int i = 0; i <= O_NCHANNELS; i++) {
                if (channel != i
                        && m_streams[channel] == m_streams[i])
                    in_use = 1;
            }
            if (!in_use)
                delete m_streams[channel];
            else if (O_INPUT == channel)
...
void Console::ShowFiles() {
    ...
    while (++n <= BTCONTENT.GetNFiles()) {
        ...
        Interact("%zu) %s [%llu] %d%%", n, BTCONTENT.GetFileName(n),
                (unsigned long long) (BTCONTENT.GetFileSize(n)),
                BTCONTENT.GetFilePieces(n) ?
                (int) (100 * tmpBitField.Count() /
                BTCONTENT.GetFilePieces(n)) : 0);
    }
}
...
void Console::Status(int immediate) {
    ...
    if (m_pre_dlrate.TimeUsed() || immediate) {
        if (m_skip_status)
            ...
            (this->*m_statusline[m_status_format]) (buffer,
                    sizeof (buffer));
            ...
            if (m_streams[O_NORMAL]->IsTTY()) {
                ...
                if (ioctl
                        (m_streams[O_NORMAL]->Fileno(), TIOCGWINSZ,
                        &tsize) >= 0)
                ...
                if (ioctl
                        (m_streams[O_NORMAL]->Fileno(), TIOCGSIZE,
                        &tsize) >= 0)
                ...
                struct uwdata tsize;
                if (ioctl
                        (m_streams[O_NORMAL]->Fileno(), WIOCGETD,
                        &tsize) >= 0)
                    tmplen =
                        tsize.uw_width / tsize.uw_hs - 1;
#endif
#endif
#endif
                if (tmplen > 80)
            ...
            if (0 == tmplen)
                tmplen =
                    (len < m_status_len) ? m_status_len : len;
            m_status_len = len;
            ...
            if (arg_verbose)
                Debug
                    ("Cache: %dK/%dM  Hits: %d  Miss: %d  %d%%  Pre: %d/%d",
                    (int) (BTCONTENT.CacheUsed() / 1024),
                    (int) cfg_cache_size,
                    (int) (BTCONTENT.CacheHits()),
                    (int) (BTCONTENT.CacheMiss()),
                    BTCONTENT.CacheHits() ? (int) (100 *
                    BTCONTENT.
                    CacheHits() /
                    (BTCONTENT.
                    CacheHits() +
...
void Console::StatusLine0(char buffer[], size_t length) {
    ...
    snprintf(buffer, length,
            "%c %d/%zu/%d [%d/%d/%d] %lluMB,%lluMB | %d,%dK/s | %d,%dK E:%d,%d %s%s",
...
void Console::StatusLine1(char buffer[], size_t length) {
    ...
    if (BTCONTENT.IsFull())
        ...
        snprintf(complete, 8, "%d/%d%%", 100 * have / all, 100 * avail / all);
    }
    ...
    if (remain >= 0) {
        ...
        if (tmpBitfield.IsEmpty())
            strncpy(timeleft, "unknown", 7);
        else
            strncpy(timeleft, "stalled", 7);
    } else
        strncpy(timeleft, "stalled", 7);

    snprintf(buffer, length,
            "%c S:%d/%d L:%zu/%zu C:%u  R=%.2f D=%d U=%d K/s  %s %s  %s%s",
            LIVE_CHAR[m_live_idx++],
            (int) (WORLD.GetSeedsCount()),
            ...
            (!BTCONTENT.IsFull() ? 1 : 0), (int) (WORLD.GetConnCount()),
            (double) (Self.TotalUL()) /
            (Self.TotalDL() ? Self.TotalDL()
            : BTCONTENT.GetTotalFilesLength()), (int) (Self.RateDL() >> 10),
            (int) (Self.RateUL() >> 10), complete, timeleft, partial,
            (Tracker.GetStatus() == T_CONNECTING) ? "Connecting"
            : ((Tracker.GetStatus() == T_READY) ? "Connected"
            : (Tracker.IsRestarting() ? "Restarting"
            : (Tracker.IsQuitting() ? "Quitting"
...
void Console::Print_n(const char *message, ...) {
    ...
    if (m_status_last && message && *message)
        Print_n("");
    m_status_last = 0;
...
void Console::Debug(const char *message, ...) {
    ...
    if (K_LINES != m_streams[O_INPUT]->GetInputMode() ||
        ...
        size_t need =
                strlen(message) + 1 + 10 * sizeof (unsigned long) / 4;
        if (need > sizeof (buffer) && (format = new char[need]))
            buflen = need;
        else {
            format = buffer;
...
void Console::Debug_n(const char *message, ...) {
    ...
    if (K_LINES != m_streams[O_INPUT]->GetInputMode() ||
        ...
        if (f_new_line && message && *message) {
            ...
            snprintf(format, buflen, "%lu %s", (unsigned long) now,
                    message);
...
void Console::cpu() {
    if (arg_verbose)
        Debug
            ...
            clock() / (double) CLOCKS_PER_SEC / (time((time_t *) 0) -
            BTCONTENT.
            GetStartTime()) * 100);
}
...
RETSIGTYPE Console::Signal(int sig_no) {
    switch (sig_no) {
        case SIGTTOU:
            for (int i = 0; i < O_NCHANNELS; i++)
                if (m_streams[i]->IsTTY())
                    m_streams[i]->Suspend();
            m_conmode = m_streams[O_INPUT]->GetInputMode();
            break;
        case SIGTTIN:
            if (m_streams[O_INPUT]->IsTTY())
                m_streams[O_INPUT]->Suspend();
            m_conmode = m_streams[O_INPUT]->GetInputMode();
            break;
        case SIGCONT:
...
void Console::Daemonize() {
    ...
    if ((r = fork()) < 0) {
        Warning(2, "warn, fork to background failed:  %s",
                strerror(errno));
        arg_daemon = 0;
        goto restorecache;
    } else if (r)
    ...
    if ((r = fork()) < 0) {
        Warning(2, "warn, final fork failed (continuing in background):  %s",
                strerror(errno));
        goto restorecache;
    } else if (r)
        exit(EXIT_SUCCESS);
    else if (arg_verbose)
        Debug("Running in daemon (background) mode.");
