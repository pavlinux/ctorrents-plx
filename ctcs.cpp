...
void Ctcs::Reset(time_t new_interval) {
    ...
    if (warn)
        CONSOLE.Warning(2, "Connection to CTCS closed");
}
...
int Ctcs::CheckMessage() {
    ...
    if (r == 0)
        return 0; // no data (should return an error)
    ...
    if (!q) {
        ...
        if (error != 0)
            CONSOLE.Warning(2,
                "warn, received nothing from CTCS:  %s",
                strerror(error));
        Reset(5);
        return -1;
    }
    ...
    while (in_buffer.Count() &&
        ...
        *s = '\0';
        ...
        if (!strncmp("SETDLIMIT", msgbuf, 9)) {
            ...
            if (proto <= CTCS_PROTOCOL)
                m_protocol = proto;
            else
                m_protocol = CTCS_PROTOCOL;
        } else if (s != msgbuf) {
            if (arg_verbose)
                CONSOLE.Debug("unknown CTCS message: %s",
                    msgbuf);
        }
...
int Ctcs::Report_Status() {
    ...
    changebw = (compset(m_ctstatus, dlrate) |
            compset(m_ctstatus, ulrate) |
            compset(m_ctstatus, dlimit) | compset(m_ctstatus, ulimit));
    ...
    if (!m_sent_ctstatus ||
        ...
        return (changebw || !m_sent_ctbw) ? Send_bw() : 0;
}
...
int Ctcs::Send_Status() {
    ...
    if (m_protocol == 1)
        snprintf(message, CTCS_BUFSIZE,
            ...
            (int) nhave, (int) ntotal,
            (int) (WORLD.Pieces_I_Can_Get()), (int) dlrate,
            (int) ulrate, (unsigned long long) dltotal,
            (unsigned long long) ultotal, (int) dlimit, (int) ulimit);
    else
        snprintf(message, CTCS_BUFSIZE,
            "CTSTATUS %d:%d/%d:%zu/%d %d/%d/%d %d,%d %llu,%llu %d,%d %d",
            (int) seeders,
            (int) (Tracker.GetSeedsCount()) -
            (BTCONTENT.IsFull() ? 1 : 0), (int) leechers,
            (int) (Tracker.GetPeersCount()) -
            Tracker.GetSeedsCount() - (!BTCONTENT.IsFull() ? 1 : 0),
            (int) (WORLD.GetConnCount()), (int) nhave, (int) ntotal,
            (int) (WORLD.Pieces_I_Can_Get()), (int) dlrate,
            (int) ulrate, (unsigned long long) dltotal,
            (unsigned long long) ultotal, (int) dlimit, (int) ulimit,
            (int) cacheused);
...
int Ctcs::Send_Config() {
    ...
    if (m_protocol >= 3) {
        ...
        if ((r = SendMessage("CTCONFIGSTART")) < 0)
            return r;
        snprintf(maxlen, sizeof (maxlen), "%u",
                (unsigned int) MAXPATHLEN);

        if ((r =
                SendMessage(ConfigMsg
                ("verbose", "B", "0", arg_verbose ? "1" : "0",
                "Verbose output [-v]",
                arg_verbose ? "Enabled" : "Disabled"))) < 0)
            return r;

        double num = BTCONTENT.GetSeedTime() ?
                (cfg_seed_hours -
                (now - BTCONTENT.GetSeedTime()) / 3600.0) : cfg_seed_hours;
        ...
        snprintf(message, CTCS_BUFSIZE,
            "CTCONFIG %d %d %f %d %d %d %d %d", (int) arg_verbose,
            (int) cfg_seed_hours, cfg_seed_ratio,
            (int) cfg_max_peers, (int) cfg_min_peers,
            BTCONTENT.GetFilter() ? atoi(BTCONTENT.
            GetFilterName()) : 0,
            (int) cfg_cache_size, WORLD.IsPaused());
    else // m_protocol == 1
        snprintf(message, CTCS_BUFSIZE,
            "CTCONFIG %d %d %f %d %d %d %d %d %d",
...
char *Ctcs::ConfigMsg(const char *name, const char *type, const char *range,
    ...
    if (!message) {
        ...
        if (!message) {
            CONSOLE.Warning(1,
                    "error, failed to allocate memory for CTCS message");
            return (char *) 0;
        }
    }
    snprintf(message, CTCS_BUFSIZE, "CTCONFIG %s %s %s %d:%s %d:%s %d:%s",
            name, type, range, (int) strlen(value), value,
            (int) strlen(short_desc), short_desc, (int) strlen(long_desc),
            long_desc);
    ...
    if (m_protocol >= 3) {
        ...
        if (0 == strcmp(name, "verbose")) {
            ...
            if (0 == BTCONTENT.GetSeedTime() ||
                    cfg_seed_hours >
                    (now - BTCONTENT.GetSeedTime()) / 3600
                    || arg >
                    (double) Self.TotalUL() /
                    (Self.TotalDL() ? Self.TotalDL() : BTCONTENT.
                    GetTotalFilesLength()))
            ...
            if (!BTCONTENT.IsFull()) {
                ...
                if (0 == strlen(valstr))
                    ...
                    arg_file_to_download =
                            new char[strlen(valstr) + 1];
                    if (!arg_file_to_download)
                        CONSOLE.Warning(1,
                            "error, failed to allocate memory for option");
                    else
                        strcpy(arg_file_to_download,
                            valstr);
                }
                BTCONTENT.SetFilter();
            }
            ...
            if (!BTCONTENT.IsFull()) {
                ...
                if (!arg_completion_exit)
                    ...
                    strcpy(arg_completion_exit, valstr);
            }
...
int Ctcs::Send_Detail() {
    ...
    if (r == 0)
        r = SendMessage((m_protocol >= 3) ? "CTFILESTART" : "CTFILES");
    ...
    if (r == 0)
        r = SendMessage((m_protocol >= 3) ? "CTFILESDONE" : "CTFDONE");
    return r;
}
...
int Ctcs::Send_Peers() {
    ...
    while (r == 0 && (peer = WORLD.GetNextPeer(peer))) {
        ...
        snprintf(message, CTCS_BUFSIZE,
                "CTPEER %s %s %c%c%c%c %d %d %llu %llu %d", txtid,
    ...
    if (r == 0)
        r = SendMessage((m_protocol >= 3) ? "CTPEERSDONE" : "CTPDONE");
    return r;
}
...
int Ctcs::Connect() {
    ...
    if (r == -1) {
        ...
        m_status = T_READY;
        if (arg_verbose)
            CONSOLE.Debug("Connected to CTCS");
        if (Send_Protocol() != 0 && errno != EINPROGRESS) {
            CONSOLE.Warning(2,
                    "warn, send protocol to CTCS failed:  %s",
                    strerror(errno));
            return -1;
        }
        if (Send_Auth() != 0 && errno != EINPROGRESS) {
            CONSOLE.Warning(2,
                    "warn, send password to CTCS failed:  %s",
                    strerror(errno));
            return -1;
        }
        if (Send_Torrent(BTCONTENT.GetPeerId(), arg_metainfo_file) < 0
                && errno != EINPROGRESS) {
            CONSOLE.Warning(2,
                    "warn, send torrent to CTCS failed:  %s",
                    strerror(errno));
            return -1;
        }
    }
...
int Ctcs::SocketReady(fd_set * rfdp, fd_set * wfdp, int *nfds,
    ...
    if (T_CONNECTING == m_status && FD_ISSET(m_sock, wfdp)) {
        ...
        if (error != 0) {
            ...
            if (Send_Torrent
                    (BTCONTENT.GetPeerId(), arg_metainfo_file) < 0
                    && errno != EINPROGRESS) {
                CONSOLE.Warning(2,
                        "warn, send torrent to CTCS failed:  %s",
                        strerror(errno));
