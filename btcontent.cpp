...
int btContent::NeedFlush() const {
    if (m_flush_failed) {
        ...
        return (m_flushq ||
            ...
            m_cache_used >=
            cfg_cache_size * 1024 * 1024 - cfg_req_slice_size +
            1)) ? 1 : 0;
    return -1;
}
...
int btContent::SeedTimeout() {
    ...
    if (Seeding() && (!m_flush_failed || IsFull())) {
        if (!m_seed_timestamp) {
            ...
            if (cfg_seed_ratio)
                CONSOLE.Print_n(" or to ratio of %.2f",
                    cfg_seed_ratio);
            CONSOLE.Print("");
        } else if (now < m_seed_timestamp)
            m_seed_timestamp = now;
        dl = (Self.TotalDL() >
                0) ? Self.TotalDL() : GetTotalFilesLength();
        if ((cfg_seed_ratio == 0 && cfg_seed_hours == 0)
                || (cfg_seed_hours > 0
                && (now - m_seed_timestamp) >=
                (cfg_seed_hours * 60 * 60)) || (cfg_seed_ratio > 0
                && cfg_seed_ratio <=
                (double) Self.TotalUL() /
                dl)) {
            if (m_flush_failed) {
