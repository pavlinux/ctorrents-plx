...
int btContent::CreateMetainfoFile(const char *mifn) {
    ...
    if (!fp) {
        CONSOLE.Warning(1, "error, open \"%s\" failed:  %s", mifn,
                strerror(errno));
        return -1;
    }
    ...
    // announce
    if (bencode_str("announce", fp) != 1)
        goto err;
    if (bencode_str(m_announce, fp) != 1)
        goto err;

    // comment
    if (arg_comment) {
        if (bencode_str("comment", fp) != 1)
            goto err;
        if (bencode_str(arg_comment, fp) != 1)
            goto err;
    }
    // created by
    if (bencode_str("created by", fp) != 1)
        goto err;
    if (bencode_str(cfg_user_agent, fp) != 1)
        goto err;

    // creation date
    if (bencode_str("creation date", fp) != 1)
        goto err;
    if (bencode_int(m_create_date, fp) != 1)
        goto err;

    // info dict
    if (bencode_str("info", fp) != 1)
        goto err;
    if (bencode_begin_dict(fp) != 1)
        goto err;
    ...
    { // Entries in dictionary must be sorted by key!
        ...
        if (m_btfiles.FillMetaInfo(fp) != 1)
            goto err;

        // piece length
        if (bencode_str("piece length", fp) != 1)
            goto err;
        if (bencode_int(m_piece_length, fp) != 1)
            goto err;
        ...
        // private
        if (arg_flg_private) {
            if (bencode_str("private", fp) != 1)
                goto err;
            if (bencode_int(1, fp) != 1)
                goto err;
        }
...
int btContent::InitialFromFS(const char *pathname, char *ann_url,
    ...
    if (m_btfiles.BuildFromFS(pathname) < 0)
        return -1;
    ...
    if (m_btfiles.GetTotalLength() % m_piece_length)
        m_npieces++;
    ...
    for (n = 0; n < m_npieces; n++) {
        if (GetHashValue(n, m_hash_table + n * 20) < 0)
            return -1;
        if (n % percent == 0 || n == m_npieces - 1) {
            CONSOLE.InteractU("Create hash table: %d/%d",
                    (int) n + 1, (int) m_npieces);
        }
    }
    return 0;
}
...
int btContent::PrintOut() {
    ...
    if (m_private)
        CONSOLE.Print("Private: %s", m_private ? "Yes" : "No");
    if (m_comment) {
    ...
    if (m_created_by)
        CONSOLE.Print("Created with: %s", m_created_by);
    m_btfiles.PrintOut();
    return 0;
}
...
#define ERR_RETURN()	{if(b) delete []b; return -1;}
    ...
    if (!(r = meta_pos("info")))
        ERR_RETURN();
    if (!(q = decode_dict(b + r, flen - r, (char *) 0)))
        ERR_RETURN();
    Sha1(b + r, q, m_shake_buffer + 28);

    // private flag
    if (meta_int("info|private", &r))
        m_private = r;

    // hash table
    if (!meta_str("info|pieces", &s, &m_hashtable_length) ||
            m_hashtable_length % 20 != 0)
        ERR_RETURN();
    ...
    if (!meta_int("info|piece length", &m_piece_length))
        ERR_RETURN();
    ...
    if (m_btfiles.BuildFromMI(b, flen, saveas) < 0)
        ERR_RETURN();
    ...
    if ((tmp = m_btfiles.CreateFiles()) < 0)
        ERR_RETURN();
    r = tmp;
    ...
    if (arg_flg_check_only) {
        ...
        if (pBRefer->SetReferFile(arg_bitfield_file) < 0) {
            ...
            CONSOLE.
                    Interact("Found bit field file; %s previous state.",
                    arg_flg_force_seed_mode ?
                    "resuming download from" : "verifying");
    ...
    if (!r) { // don't hash-check if the files were just created
        ...
        if (arg_flg_force_seed_mode) {
            CONSOLE.Warning(2,
                    "Files were not present; overriding force mode!");
        }
        ...
        *pBF = *pBRefer;
        if (pBF->IsFull()) {
            CONSOLE.
                    Interact
                    ("Skipping hash checks and forcing seed mode.");
            CONSOLE.
                    Interact
    ...
    *ptr = (unsigned char) 19;
    ptr++; // protocol string length
    memcpy(ptr, "BitTorrent protocol", 19);
    ptr += 19; //  protocol string
...
ssize_t btContent::ReadSlice(char *buf, size_t idx, size_t off, size_t len) {
    ...
    if (!m_cache_size)
        return buf ? m_btfiles.IO(buf, offset, len, 0) : 0;
    else {
        size_t len2;
        BTCACHE *p;
        ...
        p = m_cache[idx];
        while (len && p) {
            while (p && offset + len > p->bc_off
                    && !CACHE_FIT(p, offset, len)) {
                p = p->bc_next;
            }
            if (!p || !CACHE_FIT(p, offset, len))
                break;
            if (offset < p->bc_off) {
                len2 = p->bc_off - offset;
                if (CacheIO(buf, offset, len2, 0) < 0)
                    return -1;
                retval = 1;
                if (buf)
                    m_cache_miss +=
                        len2 / DEFAULT_SLICE_SIZE +
                        ((len2 %
                        DEFAULT_SLICE_SIZE) ? 1 : 0);
                else
                    m_cache_pre +=
                        len2 / DEFAULT_SLICE_SIZE +
                        ((len2 %
        ...
        if (len) {
            if (buf)
                m_cache_miss += len / DEFAULT_SLICE_SIZE +
                    ((len % DEFAULT_SLICE_SIZE) ? 1 : 0);
            else
                m_cache_pre += len / DEFAULT_SLICE_SIZE +
                    ((len % DEFAULT_SLICE_SIZE) ? 1 : 0);
...
again:
    for (p = m_cache_oldest; p && m_cache_size < m_cache_used + need;
            p = pnext) {
        pnext = p->age_next;
        if (f_flush) {
            if (arg_verbose)
        ...
        if (!p->bc_f_flush) {
            if (arg_verbose)
                CONSOLE.Debug("Expiring %d/%d/%d",
                    (int) (p->bc_off / m_piece_length),
                    (int) (p->bc_off % m_piece_length),
                    (int) (p->bc_len));

            if (m_cache_oldest == p)
                m_cache_oldest = p->age_next;
            else
    ...
    if (m_cache_size < m_cache_used + need) { // still not enough
        ...
        if (m_cache_size < m_cache_used + need && m_cache_used
            ...
            if (arg_verbose)
                CONSOLE.
                    Debug
                    ("CacheClean flushing to obtain space");
            f_flush = 1;
            goto again;
        } // else we tried...
    }
...
void btContent::CacheEval() {
    ...
    size_t unflushed = 0, dlnext, upadd = 0, upmin = 0, total;
    ...
    if (arg_verbose)
        CONSOLE.
            Debug("DL need: %dK  UL need: %dK  Cache: %dK  Used: %dK",
            (int) ((unflushed + dlnext) / 1024),
            (int) (upadd / 1024), (int) (m_cache_size / 1024),
            (int) (m_cache_used / 1024));
...
void btContent::FlushCache() {
    if (arg_verbose)
        CONSOLE.Debug("Flushing all cache");

    for (size_t i = 0; i < m_npieces; i++) {
        if (m_cache[i])
            FlushPiece(i);
        if (m_flush_failed)
            break;
        ...
        if (m_btfiles.IO(p->bc_buf, p->bc_off, p->bc_len, 1) < 0) {
            ...
            if (now >= m_flush_failed + 300) {
                if (!m_flush_failed)
                    m_cache_size +=
                        cfg_req_slice_size *
                        WORLD.GetDownloads() * 2;
                CONSOLE.Warning(1,
                        "warn, write file failed while flushing cache.");
                CONSOLE.Warning(1,
            ...
            p->bc_f_flush = 0;
            if (Seeding()) {
                for (size_t n = 1; n <= m_btfiles.GetNFiles();
                        n++)
...
void btContent::FlushQueue() {
    if (m_flushq) {
        if (arg_verbose)
            CONSOLE.Debug("Writing piece #%d to disk",
                (int) (m_flushq->idx));
...
int btContent::CachePrep(size_t idx) {
    ...
    if (m_cache_size < m_cache_used + need) {
        for (p = m_cache[idx]; p; p = p->bc_next)
            need -= p->bc_len;
        if (0 == need)
            retval = -1; // don't need to prefetch
        for (p = m_cache_oldest;
                p && m_cache_size < m_cache_used + need; p = pnext) {
            pnext = p->age_next;
            if (p->bc_off / m_piece_length == idx)
                continue;
            if (p->bc_f_flush) {
                if (arg_verbose)
                    CONSOLE.Debug("Flushing %d/%d/%d",
...
ssize_t btContent::WriteSlice(char *buf, size_t idx, size_t off, size_t len) {
    ...
    if (!m_cache_size)
        return m_btfiles.IO(buf, offset, len, 1);
    else {
        size_t len2;
        BTCACHE *p;
        ...
        while (len && p) {
            ...
            if (offset < p->bc_off) {
                len2 = p->bc_off - offset;
                if (CacheIO(buf, offset, len2, 1) < 0)
                    return -1;
                p = m_cache[idx]; // p may not be valid after CacheIO
            } else {
                if (offset > p->bc_off) {
                    ...
                    if (len2 > len)
                        len2 = len;
                    memcpy(p->bc_buf + (offset - p->bc_off),
                            buf, len2);
                } else {
                    len2 =
                            (len > p->bc_len) ? p->bc_len : len;
...
ssize_t btContent::CacheIO(char *buf, uint64_t off, size_t len, int method) {
    ...
    if (buf)
        memcpy(pnew->bc_buf, buf, len);
            ...
            && m_btfiles.IO(pnew->bc_buf, off, len, method) < 0) {
        delete[]pnew->bc_buf;
        delete pnew;
        return -1;
    }
...
int btContent::CheckNextPiece() {
    ...
    if (pBChecked->IsSet(idx)) {
        while (idx < m_npieces && pBChecked->IsSet(idx)) {
            if (arg_verbose)
                CONSOLE.Debug("Check: %u skipped", idx);
            pBChecked->Set(idx);
            ++idx;
        }
        f_checkint = 1;
    ...
    if (idx < m_npieces) {
        ...
        if (memcmp(md, m_hash_table + idx * 20, 20) == 0) {
            if (arg_verbose)
                CONSOLE.Debug("Check: %u ok", idx);
            m_left_bytes -= GetPieceLength(idx);
            pBF->Set(idx);
            WORLD.Tell_World_I_Have(idx);
        } else {
            if (arg_verbose)
                CONSOLE.Debug("Check: %u failed", idx);
            f_checkint = 1;
        }
        m_check_piece = idx + 1;
    }
...
int btContent::SeedTimeout() {
    ...
    if (Seeding() && (!m_flush_failed || IsFull())) {
        if (!m_seed_timestamp) {
            ...
            CONSOLE.Print_n("Seed for others %lu hours", (unsigned long) cfg_seed_hours);
...
void btContent::CompletionCommand() {
    ...
    if (nt || nd || nw) {
        ...
        if (nw && !getcwd(wd, MAXPATHLEN)) {
            CONSOLE.Warning(2,
                    "warn, couldn't get working directory:  %s",
                    strerror(errno));
            return;
        }
    ...
    if ((r = fork()) < 0) {
        CONSOLE.Warning(2,
                "warn, fork failed running completion command:  %s",
                strerror(errno));
    } else if (r == 0) {
...
#endif
        if (system(cmdstr) < 0)
            CONSOLE.Warning(2, "warn, failure running completion command:  %s", strerror(errno));
...
void btContent::CheckFilter() {
    ...
    if (!m_current_filter) {
        if (!IsFull())
            CONSOLE.Print("End of download files list.");
        for (BFNODE * goner = m_filters; goner;
                goner = m_current_filter) {
    ...
    if (m_current_filter != original) {
        if (m_current_filter) {
            ...
            if (GetFilter()->IsEmpty())
                ...
                CONSOLE.Print("Downloading file(s): %s",
                    m_current_filter->name);
            CONSOLE.Print("Pieces: %d (%llu bytes)",
                    (int) (tmpBitfield.Count()),
                    (unsigned long long)
                    ((tmpBitfield.Count() -
                    last) * (uint64_t) m_piece_length +
                    (last ? GetPieceLength(m_npieces - 1) :
                    0)));
        }
    }
...
BitField *btContent::GetNextFilter(BitField * pfilter) const {
    ...
    if (!pfilter)
        ...
        for (p = m_filters; p && (&(p->bitfield) != pfilter);
                p = p->next);
        if (p)
            p = p->next;
        else
