...
//  CONSOLE.Debug_n("");
	return (memcmp(&m_sin.sin_addr, &addr.sin_addr, sizeof(struct in_addr))
		== 0) ? 1 : 0;
}
...
int btPeer::Need_Remote_Data() const {
	if (BTCONTENT.Seeding() || bitfield.IsEmpty())
		return 0;
...
int btPeer::SetLocal(unsigned char s)
{
	...
	case M_CHOKE:
		...
		if (arg_verbose)
			CONSOLE.Debug("Choking %p (D=%lluMB@%dK/s)", this,
				      (unsigned long long)TotalDL() >> 20,
				      (int)(RateDL() >> 10));
		m_state.local_choked = 1;
		if (g_next_up == this)
			g_next_up = (btPeer *) 0;
		if (!reponse_q.IsEmpty())
			reponse_q.Empty();
		StopULTimer();
		if (!m_requested && BTCONTENT.IsFull()) {
			// hasn't sent a request since unchoke
			if (arg_verbose)
				CONSOLE.Debug("%p inactive", this);
			return -1;
		}
		m_requested = 0;
		break;
	case M_UNCHOKE:
		...
		if (arg_verbose)
			CONSOLE.Debug("Unchoking %p (D=%lluMB@%dK/s)", this,
				      (unsigned long long)TotalDL() >> 20,
				      (int)(RateDL() >> 10));
	...
	case M_INTERESTED:
		if (BTCONTENT.Seeding())
			return 0;
		m_standby = 0;
		if (m_state.local_interested)
			return 0;
		if (arg_verbose)
			CONSOLE.Debug("Interested in %p", this);
	...
	case M_NOT_INTERESTED:
		if (!m_state.local_interested)
			return 0;
		if (arg_verbose)
			CONSOLE.Debug("Not interested in %p", this);
...
int btPeer::RequestPiece()
{
	...
	if (m_last_req_piece < BTCONTENT.GetNPieces()
	    && tmpBitfield.Count() > 1)
	...
	if ((idx = PENDINGQUEUE.ReAssign(&request_q, tmpBitfield)) <
		...
		if (arg_verbose)
			CONSOLE.Debug("Assigning #%d to %p from Pending",
				      (int)idx, this);
	...
	if (m_cached_idx < BTCONTENT.CheckedPieces()
		...
		if (!BTCONTENT.pBF->IsSet(idx) && (!BTCONTENT.GetFilter()
						   || !BTCONTENT.GetFilter()->
						   IsSet(idx))
		    && !PENDINGQUEUE.Exist(idx)
		    && !WORLD.AlreadyRequested(idx)) {
			if (arg_verbose)
				CONSOLE.Debug("Assigning #%d to %p", (int)idx,
					      this);
	...
	if (BTCONTENT.pBF->IsEmpty()) {
		...
		if (m_latency < 60) {
			...
			if (m_last_req_piece < BTCONTENT.GetNPieces()
				...
				tmpBitfield.UnSet(m_last_req_piece);
			idx =
			    WORLD.What_Can_Duplicate(tmpBitfield, this,
						     BTCONTENT.GetNPieces());
			if (idx < BTCONTENT.GetNPieces()) {
				...
				if (peer) {
					if (arg_verbose)
						CONSOLE.Debug
						    ("Duping #%d from %p to %p",
						     (int)idx, peer, this);
					if (request_q.CopyShuffle
					    (&peer->request_q, idx) < 0)
						return -1;
					WORLD.CompareRequest(this, idx);
					BTCONTENT.pBMultPeer->Set(idx);
					return SendRequest();
	...
	if (tmpBitfield2.IsEmpty()) {
		...
		if (request_q.IsEmpty() && (peer = WORLD.Who_Can_Abandon(this))) {
			...
			if (request_q.Copy(&peer->request_q, idx) < 0)
				return -1;
			WORLD.CompareRequest(this, idx);
			BTCONTENT.pBMultPeer->Set(idx);
			if (endgame)
		...
		if (tmpBitfield3.IsEmpty())
			tmpBitfield3 = tmpBitfield2;
	...
	if (0 == r) {
		...
		case M_CHOKE:
			if (H_BASE_LEN != r)
				return -1;
			if (arg_verbose)
				CONSOLE.Debug("%p choked me", this);
			if (m_lastmsg == M_UNCHOKE
			    && m_last_timestamp <= m_choketime + 1) {
				if (PeerError(2, "Choke oscillation") < 0)
					return -1;
			}
			...
			if (g_next_dn == this)
				g_next_dn = (btPeer *) 0;
			if (!request_q.IsEmpty()) {
				BTCONTENT.pBMultPeer->
				    Set(request_q.GetRequestIdx());
				PutPending();
			}
			m_cancel_time = now;
			break;
		...
		case M_UNCHOKE:
			if (H_BASE_LEN != r)
				return -1;
			if (arg_verbose)
				CONSOLE.Debug("%p unchoked me", this);
			if (m_lastmsg == M_CHOKE
			    && m_last_timestamp <= m_choketime + 1) {
				if (PeerError(2, "Choke oscillation") < 0)
					return -1;
			}
		...
		case M_INTERESTED:
			if (H_BASE_LEN != r)
				return -1;
			if (arg_verbose)
				CONSOLE.Debug("%p is interested", this);
			m_state.remote_interested = 1;
			if (Need_Local_Data())
				WORLD.UnchokeIfFree(this);
			break;
		...
		case M_NOT_INTERESTED:
			if (H_BASE_LEN != r)
				return -1;
			if (arg_verbose)
				CONSOLE.Debug("%p is not interested", this);
		...
		case M_HAVE:
			...
			if (!BTCONTENT.pBF->IsSet(idx)
			    && !BTCONTENT.pBMasterFilter->IsSet(idx)) {
				if (m_cached_idx >= BTCONTENT.GetNPieces()
				    || m_standby || (!BTCONTENT.GetFilter()
						     || !BTCONTENT.
						     GetFilter()->IsSet(idx)))
					m_cached_idx = idx;
				if (arg_verbose && m_standby)
					CONSOLE.Debug("%p un-standby", this);
				m_standby = 0;
			}
		...
		case M_REQUEST:
			if (H_REQUEST_LEN != r || !m_state.remote_interested)
				return -1;
			...
			if (m_state.local_choked) {
				...
				if (!m_requested) {
					m_requested = 1;
					if (stream.
					    out_buffer.SetSize(BUF_DEF_SIZ +
							       (len <
								DEFAULT_SLICE_SIZE)
							       ?
							       DEFAULT_SLICE_SIZE
					...
					if ((!m_receive_time
						...
						m_latency =
						    (now <=
						     m_unchoke_timestamp) ? 1
						    : (now -
						       m_unchoke_timestamp);
						if (arg_verbose)
							CONSOLE.Debug
							    ("%p latency is %d sec (request)",
							     this,
							     (int)m_latency);
					}
				}
		...
		case M_BITFIELD:
			if ((r - H_BASE_LEN) != bitfield.NBytes()
			    || !bitfield.IsEmpty())
				return -1;
			...
			if (bitfield.IsFull()) {
				...
				if (bitfield.IsEmpty())
					CONSOLE.Debug("%p bitfield is empty",
						      this);
				else
					CONSOLE.Debug("%p bitfield has %d%%",
						      this,
						      100 * bitfield.Count() /
						      BTCONTENT.GetNPieces());
			}
		...
		default:
			if (arg_verbose)
				CONSOLE.Debug
				    ("Unknown message type %d from peer %p",
				     (int)msg, this);
		}		// switch
...
int btPeer::ReponseSlice()
{
	...
	if (BTCONTENT.global_buffer_size < len) {
		...
		BTCONTENT.global_buffer_size =
		    BTCONTENT.global_piece_buffer ? len : 0;
	}
	retval =
	    BTCONTENT.ReadSlice(BTCONTENT.global_piece_buffer, idx, off, len);
	if (retval < 0)
		return -1;
	else if (retval && cfg_cache_size)
		Self.OntimeUL(0);	// disk read delay
	...
	if (retval >= 0) {
		...
		DataSended(len,
			   nowspec.tv_sec +
			   (double)(nowspec.tv_nsec) / 1000000000);
...
int btPeer::SendRequest()
{
	...
	if (m_req_out > cfg_req_queue_length) {
		if (arg_verbose)
			CONSOLE.Debug
			    ("ERROR@5: %p m_req_out underflow, resetting",
			     this);
		m_req_out = 0;
	}
	if (ps && m_req_out < m_req_send) {
		if (arg_verbose) {
			CONSOLE.Debug_n("");
			CONSOLE.Debug_n
			    ("Requesting #%d from %p (%d left, %d slots):",
		...
		for (int i = 0; ps && m_req_out < m_req_send && i < 5;
			...
			if (first && (!RateDL()
				      || 0 >=
				      (m_req_out +
				       1) * ps->length / (double)RateDL() -
				      m_latency)) {
				request_q.SetReqTime(ps, now);
				first = 0;
			} else
				request_q.SetReqTime(ps, (time_t) 0);
			if (arg_verbose)
				CONSOLE.Debug_n(".");
			if (stream.Send_Request(ps->index, ps->offset,
						ps->length) < 0) {
				return -1;
			}
...
int btPeer::CancelPiece(size_t idx)
{
	...
	for (; ps; ps = next) {
		...
		if (cancel) {
			if (arg_verbose)
				CONSOLE.Debug("Cancelling %d/%d/%d to %p",
					      (int)(ps->index),
					      (int)(ps->offset),
					      (int)(ps->length), this);
			if (stream.Send_Cancel
			    (ps->index, ps->offset, ps->length) < 0)
				return -1;
			m_req_out--;
			if (m_req_out > cfg_req_queue_length) {
				if (arg_verbose)
					CONSOLE.Debug
					    ("ERROR@1: %p m_req_out underflow, resetting",
					     this);
				m_req_out = 0;
			}
			m_cancel_time = now;
...
int btPeer::CancelRequest()
{
	...
	for (; ps; ps = ps->next) {
		...
		if (m_req_out > cfg_req_queue_length) {
			if (arg_verbose)
				CONSOLE.Debug
				    ("ERROR@2: %p m_req_out underflow, resetting",
				     this);
			m_req_out = 0;
		}
		m_cancel_time = now;
		...
		if (idx == ps->index) {
			if (off == ps->offset && len == ps->length) {
				...
				if (cancel) {
					if (arg_verbose)
						CONSOLE.Debug
						    ("Cancelling %d/%d/%d to %p",
						     (int)idx, (int)off,
						     (int)len, this);
					if (stream.Send_Cancel(idx, off, len) <
					    0)
						return -1;
					m_req_out--;
					if (m_req_out > cfg_req_queue_length) {
...
int btPeer::ReportComplete(size_t idx, size_t len)
{
	...
	if ((r = BTCONTENT.APieceComplete(idx)) > 0) {
		if (arg_verbose)
			CONSOLE.Debug("Piece #%d completed", (int)idx);
	...
	if (WORLD.GetDupReqs() && BTCONTENT.pBMultPeer->IsSet(idx)) {
		if (WORLD.CancelPiece(idx) && arg_verbose)
			CONSOLE.Debug
			    ("Duplicate request cancelled in piece completion");
	}
...
int btPeer::PieceDeliver(size_t mlen)
{
	...
	char *msgbuf = stream.in_buffer.BasePointer();
	time_t t = (time_t) 0;
	int f_accept = 0, f_requested = 0, f_success = 1, f_count = 1, f_want =
	    1;
	...
	if (f_requested || f_accept) {
		...
		if (!BTCONTENT.pBF->IsSet(idx) &&
		    BTCONTENT.WriteSlice(msgbuf + H_LEN + H_PIECE_LEN, idx, off,
					 len) < 0) {
			CONSOLE.Warning(2,
					"warn, WriteSlice failed; is filesystem full?");
			f_success = 0;
			...
			// and endgame modes.
			if (dup || (WORLD.GetDupReqs()
				    && BTCONTENT.pBMultPeer->IsSet(idx)))
	...
	if (f_requested) {
		if (t) {
			m_latency =
			    (m_last_timestamp <=
			     t) ? 1 : (m_last_timestamp - t);
			if (arg_verbose)
				CONSOLE.Debug("%p latency is %d sec (receive)",
					      this, (int)m_latency);
	...
	return (P_FAILED == m_status) ? -1 :
	    (m_standby || !f_requested) ? 0 : RequestCheck();
}
...
int btPeer::RequestCheck()
{
	...
	if (Need_Remote_Data()) {
		if (!m_state.local_interested && SetLocal(M_INTERESTED) < 0)
			return -1;
		if (!m_state.remote_choked) {
			if (m_req_out > cfg_req_queue_length) {
				if (arg_verbose)
					CONSOLE.Debug
					    ("ERROR@4: %p m_req_out underflow, resetting",
					     this);
				m_req_out = 0;
			}
			if (request_q.IsEmpty() && RequestPiece() < 0)
				return -1;
			else if (m_req_out < m_req_send &&
				 (m_req_out < 2 || !RateDL() ||
				  1 >=
				  (m_req_out +
				   1) * request_q.GetRequestLen() /
				  (double)RateDL() - m_latency)
				 // above formula is to try to allow delay between sending batches of reqs
				 && SendRequest() < 0)
				return -1;
		}
	} else if (m_state.local_interested && SetLocal(M_NOT_INTERESTED) < 0)
		return -1;
...
int btPeer::HandShake()
{
	...
	if (r < 0) {
		if (arg_verbose)
			CONSOLE.Debug("%p: %s", this,
				      (r ==
				       -2) ? "remote closed" : strerror(errno));
		return -1;
	}
	if ((r = stream.in_buffer.Count()) < 68) {
		...
		if (r > 20) {	// ignore 8 reserved bytes following protocol ID
			if (memcmp(stream.in_buffer.BasePointer() + 20,
				   BTCONTENT.GetShakeBuffer() + 20,
				   (r < 28) ? (r - 20) : 8) != 0) {
				if (arg_verbose) {
					CONSOLE.Debug_n("");
					CONSOLE.Debug_n("peer %p gave 0x",
							this);
					for (int i = 20; i < r && i < 27; i++)
						CONSOLE.Debug_n("%2.2hx",
								(unsigned
								 short)(unsigned
									char)
								(stream.in_buffer.BasePointer()
								 [i]));
					CONSOLE.Debug_n
					    (" as reserved bytes (partial)");
				}
		...
		if (r
		    && memcmp(stream.in_buffer.BasePointer(),
			      BTCONTENT.GetShakeBuffer(),
			      (r < 48) ? r : 48) != 0) {
			if (arg_verbose) {
				CONSOLE.Debug_n("");
	...
	if (memcmp
	    (stream.in_buffer.BasePointer(), BTCONTENT.GetShakeBuffer(), 68)
	    == 0) {
		if (arg_verbose)
			CONSOLE.Debug("peer %p is myself", this);
		if (m_connect && !cfg_public_ip)
			Self.SetIp(m_sin);
	...
	if (memcmp
	    (stream.in_buffer.BasePointer() + 20,
	     BTCONTENT.GetShakeBuffer() + 20, 8) != 0) {
		if (arg_verbose) {
			CONSOLE.Debug_n("");
	...
	if (arg_verbose) {
		TextPeerID((unsigned char *)(stream.in_buffer.BasePointer() +
					     48), txtid);
	...
	if (r >= 0) {
		...
		if (BTCONTENT.Seeding()) {	// i am seed
			...
			if (0 == m_unchoke_timestamp)
				m_unchoke_timestamp = now;
			m_connect_seed = 1;
		}
		if (stream.HaveMessage())
			return RecvModule();
	}
	return r;
}
...
//    this, r, (int)(stream.PeekMessage()));
	...
	while (r = stream.HaveMessage()) {
		...
		if ((r = MsgDeliver()) == -2) {
			if (arg_verbose)
				CONSOLE.Debug("%p seed<->seed detected", this);
			m_want_again = 0;
		}
...
int btPeer::SendModule()
{
	...
	if (!reponse_q.IsEmpty() && CouldReponseSlice()) {
		...
		if (!g_next_up || g_next_up == this) {
			...
			if (arg_verbose)
				CONSOLE.Debug("%p deferring UL to %p", this,
					      g_next_up);
			if (!g_defer_up)
				g_defer_up = 1;
			WORLD.Defer();
		}
	} else if (g_next_up == this)
		g_next_up = (btPeer *) 0;
...
int btPeer::CheckSendStatus()
{
	if (g_next_up == this && !WORLD.BandWidthLimitUp(Self.LateUL())) {
		if (arg_verbose) {
			...
			if (g_defer_up)
				CONSOLE.Debug("%p skipped UL", this);
		}
		g_next_up = (btPeer *) 0;
	}
	...
	if (BTCONTENT.IsFull()) {
		...
		if (!m_state.remote_choked && m_req_out) {
			...
			if (m_receive_time < now - 2 * allowance) {
				...
				if (arg_verbose)
					CONSOLE.Debug
					    ("%p unresponsive; resetting request queue",
					     this);
				...
				CONSOLE.Debug
				    ("%p unresponsive; sending keepalive",
				     this);
...
int btPeer::IsEmpty() const {
	return (bitfield.IsEmpty()
		&& TotalUL() < BTCONTENT.GetPieceLength() * 2) ? 1 : 0;
    ...
    void btPeer::Prefetch(time_t deadline)
{
	...
	if (!BTCONTENT.IsFull() && Is_Remote_UnChoked() &&
	    m_prefetch_completion < 2 && request_q.LastSlice()
	    && (rd = RateDL()) > 0 && request_q.Peek(&idx, &off, &len) == 0
	    && m_last_timestamp + len / rd < now + WORLD.GetUnchokeInterval()
	    && Self.RateDL() > 0
	    && m_last_timestamp + len / rd <
	    now + (cfg_cache_size * 1024 * 1024 -
		   BTCONTENT.GetPieceLength(idx)) / Self.RateDL()) {
		...
		case 0:	// ready, no data flushed
			if (m_prefetch_completion || off == 0) {
				if (off + len < BTCONTENT.GetPieceLength(idx))
					BTCONTENT.ReadSlice(NULL, idx,
							    off + len,
							    BTCONTENT.GetPieceLength
							    (idx)
							    - off - len);
				m_prefetch_completion = 2;
			} else {
		...
		if (m_next_send_time < next_chance)
			predict = next_chance;
		else
...
int btPeer::PeerError(int weight, const char *message)
{
	...
	if (arg_verbose && (weight > 0 || old_count > 0))
		CONSOLE.Debug("%p error %+d (%d) %s", this, weight, m_err_count,
			      message);
