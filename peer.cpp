	...
	if (0 == r) {
		...
		case M_CHOKE:
			...
			if (g_next_dn == this)
				g_next_dn = (btPeer *) 0;
			if (!request_q.IsEmpty()) {
				BTCONTENT.pBMultPeer->Set(request_q.
							  GetRequestIdx());
				PutPending();
			}
			m_cancel_time = now;
...
int btPeer::SendRequest()
{
	...
	if (m_req_out > cfg_req_queue_length) {
		if (arg_verbose)
			CONSOLE.
			    Debug("ERROR@5: %p m_req_out underflow, resetting",
				  this);
		m_req_out = 0;
	}
	if (ps && m_req_out < m_req_send) {
		...
		for (int i = 0; ps && m_req_out < m_req_send && i < 5;
			...
			if (arg_verbose)
				CONSOLE.Debug_n(".");
			if (stream.
			    Send_Request(ps->index, ps->offset,
					 ps->length) < 0) {
				return -1;
			}
		...
		if (cancel) {
			if (arg_verbose)
				CONSOLE.Debug("Cancelling %d/%d/%d to %p",
					      ...
					      (int)(ps->length), this);
			if (stream.
			    Send_Cancel(ps->index, ps->offset, ps->length) < 0)
				return -1;
			m_req_out--;
			if (m_req_out > cfg_req_queue_length) {
				if (arg_verbose)
							...
							(unsigned
							 short)(unsigned
								char)(stream.
								      ...
								      ()[i]));
				if (r > 48) {
					TextPeerID((unsigned char *)(stream.
								     in_buffer.
								     BasePointer
								     () + 48),
						   txtid);
...
//      if(r>=0) CONSOLE.Debug("%p fed piece, now has %d bytes", this, r);
				...
				if (arg_verbose)
					CONSOLE.
					    Debug("%p waiting for DL bandwidth",
						  this);
				g_next_dn = this;
			}
	...
	if (!reponse_q.IsEmpty() && CouldReponseSlice()) {
		...
		if (!g_next_up || g_next_up == this) {
			if (!limited) {
				...
				if (arg_verbose)
					CONSOLE.
					    Debug("%p waiting for UL bandwidth",
						  this);
				g_next_up = this;
				if (g_defer_up)
					g_defer_up = 0;
