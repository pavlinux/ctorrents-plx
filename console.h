...
enum chanels {
    O_NCHANNELS = 4, // number of output channels
    O_NORMAL = 0,
    O_INTERACT = 1,
    O_WARNING = 2,
    O_DEBUG = 3,
...
public:
    ...
    int GetMode() const {
        return m_filemode ? 1 : 0;
    }

    int Fileno() const {
        return m_stream ? fileno(m_stream) : -1;
    }

    int GetNewline() const {
        return m_newline ? 1 : 0;
    }

    void SyncNewline(ConStream *master) {
        m_newline = master->GetNewline();
    }
...
public:
    ...
    char *GetChannel(int channel) const {
