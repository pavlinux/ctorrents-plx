...
public:
    ...
    const Rate &GetDLRate() const {
        return rate_dl;
    }

    const Rate &GetULRate() const {
        return rate_ul;
    }
    ...
    uint64_t TotalDL() const {
        return rate_dl.Count();
    }

    uint64_t TotalUL() const {
        return rate_ul.Count();
    }

    void DataRecved(size_t nby) {
        rate_dl.CountAdd(nby);
    }

    void DataUnRec(size_t nby) {
        rate_dl.UnCount(nby);
    }

    void DataSended(size_t nby, double timestamp) {
        rate_ul.CountAdd(nby);
        rate_ul.RateAdd(nby, (size_t) cfg_max_bandwidth_up, timestamp);
    }

    size_t CurrentDL() {
        return rate_dl.CurrentRate();
    }

    size_t CurrentUL() {
        return rate_ul.CurrentRate();
    }

    size_t RateDL() {
        return rate_dl.RateMeasure();
    }

    size_t RateUL() {
        return rate_ul.RateMeasure();
    }

    size_t NominalDL() {
        return rate_dl.NominalRate();
    }

    size_t NominalUL() {
        return rate_ul.NominalRate();
    }
    ...
    double LastSendTime() const {
        return rate_ul.LastRealtime();
    }

    double LastRecvTime() const {
        return rate_dl.LastRealtime();
    }

    size_t LastSizeSent() const {
        return rate_ul.LastSize();
    }

    size_t LastSizeRecv() const {
        return rate_dl.LastSize();
    }
    ...
    int OntimeDL() const {
        return rate_dl.Ontime();
    }

    int OntimeUL() const {
        return rate_ul.Ontime();
    }

    void OntimeDL(int yn) {
        rate_dl.Ontime(yn);
    }

    void OntimeUL(int yn) {
        rate_ul.Ontime(yn);
    }
};
...
private:
    ...
    time_t m_health_time;
    time_t m_receive_time;
    time_t m_next_send_time;
    char m_lastmsg;
    time_t m_choketime;
...
public:
    ...
    time_t SetLastTimestamp() {
        return time(&m_last_timestamp);
    }
    ...
    time_t SetLastUnchokeTime() {
        return time(&m_unchoke_timestamp);
    }

    time_t GetLastUnchokeTime() const {
        return m_unchoke_timestamp;
    }

    int Is_Remote_Interested() const {
        return m_state.remote_interested ? 1 : 0;
    }

    int Is_Remote_UnChoked() const {
        return m_state.remote_choked ? 0 : 1;
    }

    int Is_Local_Interested() const {
        return m_state.local_interested ? 1 : 0;
    }

    int Is_Local_UnChoked() const {
        return m_state.local_choked ? 0 : 1;
    }
    ...
    int Retried() const {
        return m_retried ? 1 : 0;
    }

    int ConnectedWhileSeed() const {
        return m_connect_seed ? 1 : 0;
    }
