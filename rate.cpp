...
BWSAMPLE *Rate::NewSample()
{
	...
	if (sample) {
		...
		CONSOLE.Warning(2,
		"warn, failed to allocate memory for bandwidth sample.");
	...
	while (p && RATE_INTERVAL <= now - (time_t) (p->timestamp)) {
		...
		if (!p->next) {
			if (BWSAMPLE * q = NewSample()) {
				...
				p->bytes =
					p->bytes * RATE_INTERVAL / (now -
					(time_t) (p->
					timestamp));
				p->timestamp =
					(double) (now - RATE_INTERVAL + 1);
			}
		}
		if (p->next) {
			if (RATE_INTERVAL > now - (time_t) (p->next->timestamp)) {
				time_t reftime =
					nzero ? now : (time_t) (p->next->timestamp);
				while ((time_t) (p->next->timestamp) >
					(time_t) (p->timestamp) + 1) {
					// fill holes
					if (BWSAMPLE * q = NewSample()) {
						q->timestamp =
							(double) ((time_t)
...
void Rate::RateAdd(size_t nbytes, size_t bwlimit, double timestamp)
{
	...
	if (!m_selfrate && m_ontime) {
		double late =
			timestamp - (m_last_realtime +
			(double) m_last_size / bwlimit);
		//  double tmplate = late;
		// keep the change under control in case the system gets weird on us
		if (late < 0)
			late /= 2;
		else if (m_late && late > m_late)
			late = m_late / 2;
		m_late += late;
...
size_t Rate::RateMeasure()
{
	...
	if (now == (time_t) m_recent_realtime) {
		...
		(double) m_recent_size / ((double) now - m_recent_realtime) >
		(double) m_prev_size / (m_recent_realtime - m_prev_realtime)) {
		// "tone down" the most recent to match the previous addition's rate
		countbytes -= m_recent_size;
		countbytes +=
			(unsigned long) (m_prev_size /
			(m_recent_realtime -
			m_prev_realtime) * (now -
			(time_t)
			m_recent_realtime));
